...
export default function createOrRecover() {
  ...
  const createOrFetchWallet = useCallback(async (user, userId) => {
    const myHeaders = new Headers();
    myHeaders.append("Content-Type", "application/json");
    const raw = JSON.stringify({
      user,
  ...
  const createWallet = useCallback(async (image, userId) => {
    setLoading(true);
    ...
    if (faceResult === null) {
      ...
      if (typeof faceResult === "string") {
        const { result: walletResult } = await createOrFetchWallet(faceResult, userId);
        ...
        const { result: walletResult } = await createOrFetchWallet(nonce, userId);
        if (walletResult !== null) {
          const { user, address } = walletResult;
          setEncryptedStorageValue({ user });
          setAsyncStorageValue({ address });
      ...
      >
        ...
        >
          <CamFace
            facing={"front"}
            take={take}
            onImage={(image) => createWallet(image, profile.userId)}
          />
        </View>
        <Pressable
          disabled={loading}
